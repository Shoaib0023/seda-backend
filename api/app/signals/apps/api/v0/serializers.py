...
class SignalCreateSerializer(serializers.ModelSerializer):
    location = _NestedLocationModelSerializer()   #extra
    reporter = _NestedReporterModelSerializer()
    status = _NestedStatusModelSerializer()      # extra
    category = _NestedCategoryModelSerializer(source='category_assignment') #extra
